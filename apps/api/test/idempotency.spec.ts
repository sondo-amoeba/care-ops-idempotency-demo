...
describe("Care-ops idempotency (e2e)", () => {
  ...
  it("write-only request path: send claims a pending row, replays dedupe to it (ADR-0006)", async () => {
    const created = await request(app.getHttpServer())
      .post("/care-ops/interactions")
      .send({ patientId: "p-writeonly", programId: "behavioral-health-outreach" })
      .expect(201);

    const interactionId = created.body.id as string;
    const sendBody = {
      interactionId,
      patientId: "p-writeonly",
      programId: "behavioral-health-outreach",
      templateId: "writeonly-template",
      body: "The relay will submit this, not the request.",
    ...
    // Request path does NOT call the carrier — the row is durable and pending.
    expect(first.body.duplicate).toBe(false);
    expect(first.body.carrierSubmitted).toBe(false);
    expect(first.body.message.status).toBe("pending");
    expect(first.body.outbox.status).toBe("pending");
    ...
    expect(replay.body.message.status).toBe("pending");
  });

  it("explicit resend with a new resendKey claims a second pending row", async () => {
    const created = await request(app.getHttpServer())
      .post("/care-ops/interactions")
    ...
    const original = await request(app.getHttpServer())
      ...
      .send(base)
      .expect(201);
    expect(original.body.duplicate).toBe(false);

    // Same key replays as a duplicate (same intent)...
    const replay = await request(app.getHttpServer())
      .post("/care-ops/sms/send")
      .send(base)
      .expect(201);
    ...
    // ...a new resendKey is a NEW intent -> a second row.
